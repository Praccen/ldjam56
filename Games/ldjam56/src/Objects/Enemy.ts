...
export default class Enemy {
  ...
  updateTargetPos() {
    if (
      vec3.distance(this.physicsObj.transform.position, this.targetPos) < 3.5
    ) {
    ...
    this.enemies.forEach((otherEnemy) => {
      if (otherEnemy != this) {
        ...
        if (distance < this.separationRadius) {
          ...
          if (
            vec3.dot(
              vec3.normalize(vec3.create(), otherEnemy.physicsObj.velocity),
  ...
  smoothTurnTowardsTarget(targetDirection: vec3) {
    ...
    quat.rotationTo(targetRotation, vec3.fromValues(0, 0, 1), targetDirection);
    ...
    if (this.player.physicsObj != undefined) {
      ...
      if (distance < playerSpottingDistance) {
        ...
        if (
          ...
          if (
            hitObject != undefined &&
            hitObject.physicsObjectId === this.player.physicsObj.physicsObjectId
          ) {
  ...
  preRenderingUpdate(dt: number) {
    if (this.animatedMesh != undefined) {
      ...
      if (
        ...
        this.animatedMesh.graphicsObjectAndGltfObject.gltfObject != undefined
      ) {
        let handIndex =
        ...
        if (handIndex != undefined) {
          let node =
            this.animatedMesh.graphicsObjectAndGltfObject.gltfObject.nodes[
              handIndex
            ];
          ...
          this.lanternMesh.transform.position = vec3.transformMat4(
            vec3.create(),
            vec3.create(),
            mat
          );
