import { Camera, Div, GraphicsBundle, GUIRenderer, quat, Renderer2D, Scene, TextObject2D, vec2, vec3} from "praccen-web-engine";
import { Input } from "../Input.js";
import { mat2 } from "gl-matrix";
import { ItemList, ItemSpec, Rarity, rarityColourMap } from "../Objects/Items/ItemSpecs.js";
...
export class Item {
    ...
    constructor(spec: ItemSpec, graphicsBundle: GraphicsBundle) {
        ...
        this.spec = {name: spec.name, description: spec.description, texturePath: spec.texturePath, rarity: spec.rarity, positions: []};
    ...
    rotate() {
        ...
        quat.rotateZ(this.graphicsBundle.transform.rotation, this.graphicsBundle.transform.rotation, -Math.PI * 0.5);
    }
    ...
    setupGraphicsBundle(stride: number) {
        ...
        vec3.set(this.graphicsBundle.transform.origin, -0.5 + 1.0 / (xMax - xMin + 1) * 0.5, 0.5 - 1.0 / (yMax - yMin + 1) * 0.5, 0.0);
        vec3.set(this.graphicsBundle.transform.scale, stride * (xMax - xMin + 1), stride * (yMax - yMin + 1), 0.1);
...
export default class Inventory {
    ...
    constructor(guiRenderer: GUIRenderer) {
        ...
        this.inventoryDiv.getElement().appendChild(this.inventoryRenderer.domElement);
    ...
    private async createGrid() {
        ...
        for (let x = 0; x < this.width; x++) {
            ...
            for (let y = 0; y < this.height; y++) {
                let mesh = await this.inventoryScene.addNewMesh("Assets/objs/cube.obj", "CSS:" + emptyTileColour, "CSS:rgb(0,0,0)");
                vec3.set(mesh.transform.scale, this.boxSide, this.boxSide, 0.1);
                vec3.set(mesh.transform.position, -0.9 + this.stride * 0.5 + x * this.stride, 0.9 - this.stride * 0.5 - y * this.stride, -1.0);
    ...
    resize(width: number, height: number) {
        ...
        let h = 4;
        let w = 4 * aspectRatio; 
        this.inventoryCamera.setOrthogonalDimensions(w * -0.5, w * 0.5, h * -0.5, h * 0.5);
    }
    ...
    private tryPlaceItem(item: Item, x: number, y: number, itemIndex?: number): boolean {
        ...
        for (let position of item.spec.positions) {
            ...
            this.grid[x + position[0]][y + position[1]].gb.diffuse = this.inventoryRenderer.textureStore.getTexture(colour);
        }

        vec3.set(item.graphicsBundle.transform.position, -0.9 + (x + 0.5) * this.stride, 0.9 - (y + 0.5) * this.stride, 0.0);
    ...
    private getCoordsFromMousePosition(): vec2 {
        ...
        if (inventoryCoords[0] > -0.9 && inventoryCoords[0] < 0.9 && inventoryCoords[1] > -0.9 && inventoryCoords[1] < 0.9) {
            ...
            for (let tx = 0; tx < this.width; tx++) {
                for (let ty = 0; ty < this.height; ty++) {
                    if (this.grid[tx][ty].itemNr == selectedItem) {
                        this.grid[tx][ty].gb.diffuse = this.inventoryRenderer.textureStore.getTexture("CSS:" + emptyTileColour);
    ...
    private placeItemAndPickUpBlockingItem(coords: vec2): boolean {
        const itemsInTheWay = this.getItemsInTheWay(this.items[this.holdingItemId], coords[0], coords[1]);
        ...
        if (!this.tryPlaceItem(this.items[this.holdingItemId], coords[0], coords[1], this.holdingItemId)) {
    ...
    private storeItem(item: Item): boolean {
        if (this.grid.length < this.width || this.grid[this.width - 1].length < this.height) {
            return false;
        }
    ...
    async addItem(itemSpec: ItemSpec): Promise<Item> {
        return new Promise<Item>((resolve, reject) => {
            this.inventoryScene.addNewMesh("Assets/objs/cube.obj", "CSS:rgba(0, 0, 0, 0.0)", "CSS:rgb(0,0,0)").then((gb) => {
                let item = new Item(itemSpec, gb);
                item.setupGraphicsBundle(this.stride);
                gb.diffuse = this.inventoryRenderer.textureStore.getTexture(item.spec.texturePath);
    ...
    update(dt: number) {
            ...
            if (!this.mouseLeftWasPressed) {
                if (this.holdingItemId < 0) {
                    ...
                    // Couldn't place the item directly. Check if it's only colliding with one other item. If so - place the currently held item and pick up the new one.
                        ...
                        this.inventoryScene.deleteGraphicsBundle(this.items[this.holdingItemId].graphicsBundle);
        ...
        for (let x = 0; x < this.width; x++) {
            for (let y = 0; y < this.height; y++) {
                if (this.grid[x][y].itemNr < 0) {
                    this.grid[x][y].gb.diffuse = this.inventoryRenderer.textureStore.getTexture("CSS:" + emptyTileColour);
                }
            }
        }
        ...
        if (this.holdingItemId >= 0) {
            ...
            if (this.testItemPlacement(this.items[this.holdingItemId], coords[0], coords[1])) {
                for (let position of item.spec.positions) {
                    this.grid[coords[0] + position[0]][coords[1] + position[1]].gb.diffuse = this.inventoryRenderer.textureStore.getTexture("CSS:rgb(255,255,255)");
                }
            }
            vec3.set(item.graphicsBundle.transform.position, inventoryCoords[0], inventoryCoords[1], 0.5);
            item.graphicsBundle.layer = 2;
        }
        else {
            if (coords[0] >= 0 &&  coords[0] < this.width && coords[1] >= 0 && coords[1] < this.height) {
                ...
                if (itemIndex > -1) {
                    this.toolTip.getElement().innerHTML = 
                        ...
                        + Rarity[this.items[itemIndex].spec.rarity] + "</span>";
                    showToolTip = true;
                }
            }
        }
