...
export module LabyrinthGenerator {
	...
	export function getLabyrinth(
		...
		if (mustGoRooms.some((room) => {noGoRooms.includes(room)})) {
		...
		for (const room of mustGoRooms) {
			if (room[0] < 0 || room[0] >= xSize || room[1] < 0 || room[1] >= ySize) {
				console.error("Labyrinth generator: mustGoRooms includes a room outside of the grid");
				return null;
			}
		}
		...
		generateMaze(map, xSize, ySize, mustGoRooms, noGoRooms, connectionRooms, keepWallChance);
	...
	function calculateWallNumbersNextToRooom(x: number, y: number, xSize: number, ySize: number, nrWalls: number): {up: number, down: number, left: number, right: number} {
		let returnObject: {up: number, down: number, left: number, right: number} = {up: null, down: null, left: null, right: null};
		...
		*/
		...
		if (returnObject.right < 0 || returnObject.right >= nrWalls || x >= (xSize - 1) / 2 - 1) {
			returnObject.right = null;
		}
		if (returnObject.up < 0 || returnObject.up >= nrWalls || y <= 0) {
			returnObject.up = null;
		}
		if (returnObject.down < 0 || returnObject.down >= nrWalls || y >= (ySize - 1) / 2 - 1) {
	...
	function generateMaze(
		...
		for (const noGoRoom of noGoRooms) {
			if (noGoRoom[0] >= 0 && noGoRoom[0] < ((xSize - 1) / 2)
			&& noGoRoom[1] >= 0 && noGoRoom[1] < ((ySize - 1) / 2)) {
		...
		if (mustGoRooms.length > 0) {
			firstMustGoRoomNr = mustGoRooms[0][1] * ((xSize - 1) / 2) + mustGoRooms[0][0];
			// go until all mustGoRooms are visisted
			let done = false;
			while (!done) {
				done = true;
				for (let i = 0; i < mustGoRooms.length; i++) {
					if (spaces.find(mustGoRooms[0][1] * ((xSize - 1) / 2) + mustGoRooms[0][0]) != spaces.find(mustGoRooms[i][1] * ((xSize - 1) / 2) + mustGoRooms[i][0])) {
						done = false; 
						break;
					}
				}
		...
		for (const connectionRoom of connectionRooms) {
			if (connectionRoom[0] >= 0 && connectionRoom[0] < ((xSize - 1) / 2)
			&& connectionRoom[1] >= 0 && connectionRoom[1] < ((ySize - 1) / 2)) {
				for (let x = -1; x < 2; x++) {
					for (let y = -1; y < 2; y++) {
						if (x != 0 && y != 0) {
							continue;
						...
						if (connectionRoom[0] + x >= 0 && connectionRoom[0] + x < ((xSize - 1) / 2)
						&& connectionRoom[1] + y >= 0 && connectionRoom[1] + y < ((ySize - 1) / 2)) {
							let connectionRoomNr = connectionRoom[1] * ((xSize - 1) / 2) + connectionRoom[0];
							let adjecentRoomNr = (connectionRoom[1] + y) * ((xSize - 1) / 2) + connectionRoom[0] + x;
							if (spaces.set[adjecentRoomNr] != undefined) {
									if (spaces.findCompress(connectionRoomNr) != spaces.findCompress(adjecentRoomNr)) {
										// Figure out which tree is correct
										if (firstMustGoRoomNr >= 0 && spaces.find(connectionRoomNr) == spaces.find(firstMustGoRoomNr)) {
									...
									// Also remove wall
									let wallNrs = calculateWallNumbersNextToRooom(connectionRoom[0], connectionRoom[1], xSize, ySize, nrOfWalls);
		...
		for (let column = 0; column < xSize; column++) {
			for (let row = 0; row < ySize; row++) {
				if (map[column][row] == 1 && !checkIfNextToInaccessible(column, row) && Math.random() > keepWallChance) { 
					// Remove 
					map[column][row] = 0;
				}
			}
		...
		for (let column = 0; column < xSize; column++) {
			for (let row = 0; row < ySize; row++) {
				if (map[column][row] <= 0) { // Room / opening
					continue;
				}
				
				if (!checkAbove(column, row) && !checkBelow(column, row) && checkLeft(column, row) && checkRight(column, row)) {
					map[column][row] = 1; // ─
				}
	...
	function markEmptySpaces(
		...
		for (let y = 0; y < ySize; y++) {
			for (let x = 0; x < xSize; x++) {
				if (map[x][y] > 2) {
					if (startSet != undefined && spaces.find(map[x][y] - 3) == spaces.find(startSet)) {
						// There is a startSet and the xy set shares parent with it 
						map[x][y] = 0;
					}
					else if (startSet == undefined && spaces.find(map[x][y] - 3) != undefined) {
	...
	function removeRandomWall(
		...
		if (walls[randWall] != 0 && tested[randWall] != 1) {
			...
			if (
				spaces.findCompress(elements[0]) != undefined && spaces.findCompress(elements[1]) != undefined
	...
	export function getAsciiMap(map: Array<Array<number>>, visitedRooms?: Set<string>): string {
		if (map == undefined) {
			return;
		}
				...
				if (visitedRooms != undefined) {
					...
					for (let xOffset = -2; xOffset < 3 && !visited; xOffset++) {
						for (let yOffset = -2; yOffset < 3 && !visited; yOffset++) {
							if (visitedRooms.has((i + xOffset - 1) / 2.0 + ";" + (j + yOffset - 1) / 2.0)) {
								visited = true;
							}
						}
					}
