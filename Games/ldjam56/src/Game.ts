import {vec3, vec2, Renderer3D, GUIRenderer, Scene, Camera, PhysicsScene, AnimatedGraphicsBundle, MousePicking, quat} from "praccen-web-engine"
...
for (let i = 0; i < 2; i++) {
    ...
    if (pointLightPosCookie != "") {
        vec3.set(pointLights[i].position, parseFloat(pointLightPosCookie.split(":")[0]), parseFloat(pointLightPosCookie.split(":")[1]), parseFloat(pointLightPosCookie.split(":")[2]));
    }
}
...
if (camPosCookie != "") {
    camera.setPosition(vec3.fromValues(parseFloat(camPosCookie.split(":")[0]), parseFloat(camPosCookie.split(":")[1]), parseFloat(camPosCookie.split(":")[2])));
}
if (camDirCookie != "") {
...
scene.addNewAnimatedMesh("", "CSS:rgb(255, 0, 0)", "CSS:rgb(0,0,0)").then((aMeshBundle) => {
...
function update(dt: number) {
    ...
    if (inventory.enabled) {
        ...
        while (inventory.droppedItems.length > 0) {
            itemHandler.dropItem(inventory.droppedItems.pop(), player.physicsObj.transform.position)
        }
        ...
        if (Input.keys["O"]) {
            if (!oWasPressed) {
                itemHandler.dropItem(ItemList[Math.round(Math.random() * (ItemList.length - 1))], player.physicsObj.transform.position);
            }
            oWasPressed = true;
        }
        else {
        ...
        if (player.physicsObj != undefined) {
            map.updatePhysicsObjects(vec2.fromValues(player.physicsObj.transform.position[0], player.physicsObj.transform.position[2]));
        }
    }    
}
...
function preRendereringUpdate(dt: number) {
    if (gui.cameraFollowCheckbox.getChecked() && player.physicsObj != undefined) {
        ...
        camera.setPosition(vec3.add(vec3.create(), player.physicsObj.transform.position, offsetVec));
        camera.setDir(vec3.negate(vec3.create(), offsetVec));
        scene.getDirectionalLight().shadowFocusPos = vec3.fromValues(map.focusRoom[0] * 10.0 + 5.0, 0.0, map.focusRoom[1] * 10.0 + 5.0);
    }
    else {
        // Move camera with WASD (W and S will move along the direction of the camera, not along the xz plane)
        const cameraSpeed = 10.0;
        if (Input.keys["W"]) {
            camera.translate(vec3.scale(vec3.create(), camera.getDir(), cameraSpeed * dt));
        }
        if (Input.keys["S"]) {
            camera.translate(vec3.scale(vec3.create(), camera.getDir(), -cameraSpeed * dt));
        }
        if (Input.keys["D"]) {
            camera.translate(vec3.scale(vec3.create(), camera.getRight(), cameraSpeed * dt));
        }
        if (Input.keys["A"]) {
            camera.translate(vec3.scale(vec3.create(), camera.getRight(), -cameraSpeed * dt));
        }
        ...
        if (Input.mouseClicked
            ...
            && gui.volumetricRenderScaleSlider.getInputElement() != document.activeElement) { // Make sure the user is not changing a slider
    ...
    if (Input.keys["E"]) { 
        ...
        if (dist < Infinity) {
            vec3.scaleAndAdd(pointLights[pointLightIndex].position, camera.getPosition(), ray.getDir(), dist);
    ...
    scene.getDirectionalLight().ambientMultiplier = gui.ambientSlider.getValue() * 0.01;

    // Update the clear colour to match the ambience
    renderer.clearColour.r = scene.getDirectionalLight().colour[0] * (0.44 * scene.getDirectionalLight().ambientMultiplier);
    renderer.clearColour.g = scene.getDirectionalLight().colour[1] * (0.44 * scene.getDirectionalLight().ambientMultiplier);
    renderer.clearColour.b = scene.getDirectionalLight().colour[2] * (0.44 * scene.getDirectionalLight().ambientMultiplier);

    // Update ascii map
    if (player.physicsObj != undefined) {
        gui.mapDisplay.textString = map.getAsciiMapOfVisitedRooms();
        let pitchJaw = camera.getPitchJawDegrees();
        gui.mapDisplay.transformString = "translate(-" + ((player.physicsObj.transform.position[0] + 2.5) / (map.getMapSize()[0] + 5.0)) * 100.0 + "%, -" + ((player.physicsObj.transform.position[2] + 2.5) / (map.getMapSize()[1] + 5.0)) * 100.0 + "%) ";
        gui.mapDisplay.transformString += "rotate(" + (pitchJaw[1] + 180.0) + "deg) ";
        gui.mapDisplay.getElement().style.transformOrigin = "" + ((player.physicsObj.transform.position[0] + 2.5) / (map.getMapSize()[0] + 5.0)) * 100.0 + "% " + ((player.physicsObj.transform.position[2] + 2.5) / (map.getMapSize()[1] + 5.0)) * 100.0 + "% "
...
window.addEventListener("beforeunload", function (e: BeforeUnloadEvent) {
    SetCookie("camPos", camera.getPosition()[0] + ":" + camera.getPosition()[1] + ":" + camera.getPosition()[2]);
    SetCookie("camDir", pitch + ":" + jaw);
    SetCookie("volumetricRenderScale", gui.volumetricRenderScaleSlider.getValue());
    ...
    for (let i = 0; i < pointLights.length; i++) {
        SetCookie("pointLights[" + i + "]Pos", pointLights[i].position[0] + ":" + pointLights[i].position[1] + ":" + pointLights[i].position[2]);
    }
