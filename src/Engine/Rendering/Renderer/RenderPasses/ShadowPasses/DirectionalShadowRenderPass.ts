...
export default class DirectionalShadowRenderPass {
	...
	constructor(gl: WebGL2RenderingContext, directionalShadowShaderProgram: DirectionalShadowShaderProgram, directionalShadowInstancedShaderProgram: DirectionalShadowInstancedShaderProgram, directionalShadowSkeletalAnimationShaderProgram: DirectionalShadowSkeletalAnimationShaderProgram) {
		this.gl = gl;
		this.directionalShadowShaderProgram = directionalShadowShaderProgram;
		this.directionalShadowInstancedShaderProgram = directionalShadowInstancedShaderProgram;
		this.directionalShadowSkeletalAnimationShaderProgram = directionalShadowSkeletalAnimationShaderProgram;
		// ---- Shadow mapping ----
		this.shadowResolution = 800;
		this.shadowBuffer = new Framebuffer(
			this.gl,
			this.shadowResolution,
			this.shadowResolution,
			[],
			new Texture(this.gl, false, this.gl.DEPTH_COMPONENT32F, this.gl.DEPTH_COMPONENT, this.gl.FLOAT)
		);
		// ------------------------
		
		this.shadowBuffer.depthTexture.setTexParameterI(this.gl.TEXTURE_WRAP_S, this.gl.CLAMP_TO_EDGE);
		this.shadowBuffer.depthTexture.setTexParameterI(this.gl.TEXTURE_WRAP_T, this.gl.CLAMP_TO_EDGE);
		this.frameCounter = 0;
	}
	...
	draw(scene: Scene) {
		...
		this.gl.viewport(1, 1, this.shadowResolution - 2, this.shadowResolution - 2); // Leave one pixel at the border so that clamp to edge is consistent
		...
		scene
			...
			.calcAndSendLightSpaceMatrix(
				this.gl,
				this.directionalShadowShaderProgram.getUniformLocation("lightSpaceMatrix")[0]
			);
		...
		scene
			...
			.sendLightSpaceMatrix(
				this.gl,
				this.directionalShadowInstancedShaderProgram.getUniformLocation("lightSpaceMatrix")[0]
			);
		scene.renderSceneInstanced(this.directionalShadowInstancedShaderProgram, false);
		...
		scene
			...
			.sendLightSpaceMatrix(
				this.gl,
				this.directionalShadowSkeletalAnimationShaderProgram.getUniformLocation("lightSpaceMatrix")[0]
			);
		scene.renderSceneAnimated(this.directionalShadowSkeletalAnimationShaderProgram, false);
