...
export default class SkyboxRenderPass {
	...
	constructor(gl: WebGL2RenderingContext, skyboxShaderProgram: SkyboxShaderProgram) {
		this.gl = gl;
	...
	draw(camera: Camera) {
		if (this.skybox != undefined) {
			...
			this.skybox.shaderProgram.use();
			const viewProjMatrixUL = this.skyboxShaderProgram.getUniformLocation("viewProjMatrix");
