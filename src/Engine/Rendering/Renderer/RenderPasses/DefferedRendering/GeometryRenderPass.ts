...
export default class GeometryRenderPass {
	...
	constructor(gl: WebGL2RenderingContext, geometryPassShaderProgram: GeometryPassShaderProgram, geometryPassInstancedShaderProgram: GeometryPassInstancedShaderProgram, geometryPassSkeletalAnimationShaderProgram: GeometryPassSkeletalAnimationShaderProgram) {
		this.gl = gl;
		this.geometryPassShaderProgram = geometryPassShaderProgram;
		this.geometryPassInstancedShaderProgram = geometryPassInstancedShaderProgram;
		this.geometryPassSkeletalAnimationShaderProgram = geometryPassSkeletalAnimationShaderProgram;
		this.outputFramebuffer = new Framebuffer(this.gl,
			100,
			100,
			[
				new Texture(this.gl, false, this.gl.RGBA32F, this.gl.RGBA, this.gl.FLOAT),
				new Texture(this.gl, false, this.gl.RGBA32F, this.gl.RGBA, this.gl.FLOAT),
	...
	draw(scene: Scene, camera: Camera) {
		...
		this.gl.viewport(0.0, 0.0, this.outputFramebuffer.getWidth(), this.outputFramebuffer.getHeight());
		...
		camera.bindViewProjMatrix(this.gl, this.geometryPassShaderProgram.getUniformLocation("viewProjMatrix")[0]);

		scene.renderScene(this.geometryPassShaderProgram, true);

		this.geometryPassInstancedShaderProgram.use();
		camera.bindViewProjMatrix(this.gl, this.geometryPassInstancedShaderProgram.getUniformLocation("viewProjMatrix")[0]);
		...
		camera.bindViewProjMatrix(this.gl, this.geometryPassSkeletalAnimationShaderProgram.getUniformLocation("viewProjMatrix")[0]);

		scene.renderSceneAnimated(this.geometryPassSkeletalAnimationShaderProgram, true);
	}
}
