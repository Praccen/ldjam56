...
export default class ParticleRenderPass {
	...
	constructor(gl: WebGL2RenderingContext, particleShaderProgram: ParticleShaderProgram) {
		this.gl = gl;
	...
	draw(scene: Scene, camera: Camera, rendererStartTime: number) {
		...
		if (scene.particleSpawners.length > 0) {
			...
			camera.bindViewProjMatrix(this.gl, this.particleShaderProgram.getUniformLocation("viewProjMatrix")[0]);
			this.gl.uniform3fv(this.particleShaderProgram.getUniformLocation("cameraPos")[0], camera.getPosition());
			this.gl.uniform1f(
