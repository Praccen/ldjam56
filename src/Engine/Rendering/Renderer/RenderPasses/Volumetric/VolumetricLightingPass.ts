...
export default class VolumetricLightingPass {
	...
	directionalDepthMap: Texture) {
		...
		this.width = 100;
		this.height = 100;
		this.volumetricScreenQuad = new ScreenQuad(this.gl, [positionTexture, fogTexture, directionalDepthMap]);
		this.upscaleFramebuffer = new Framebuffer(this.gl, 100, 100, [new Texture(this.gl, false)], null);
		this.combineScreenQuad = new ScreenQuad(this.gl, this.upscaleFramebuffer.textures);
	}

	setResolution(x: number, y: number) {
		this.width = x;
		this.height = y;
		this.upscaleFramebuffer.setProportions(x * this.renderScale, y * this.renderScale);
	}

	setRenderScale(renderScale) {
		if (renderScale != this.renderScale) {
			this.renderScale = renderScale;
			this.upscaleFramebuffer.setProportions(this.width * this.renderScale, this.height * this.renderScale);
		}
	}
	...
	draw(scene: Scene, camera: Camera, rendererStartTime: number) {
		...
		this.gl.viewport(0, 0, this.upscaleFramebuffer.getWidth(), this.upscaleFramebuffer.getHeight());
		...
		camera.bindViewProjMatrix(
			this.gl,
			this.volumetricLightingShaderProgram.getUniformLocation("viewProjMatrix")[0]
		);
		this.gl.uniform3fv(
		...
		scene.directionalLight.sendLightSpaceMatrix(
			this.gl,
			this.volumetricLightingShaderProgram.getUniformLocation("lightSpaceMatrix")[0]
		);

		this.gl.uniform1f(this.volumetricLightingShaderProgram.getUniformLocation("fogMaxDistance")[0], this.fogMaxDistance);
		this.gl.uniform1f(this.volumetricLightingShaderProgram.getUniformLocation("fogDensity")[0], this.fogDensity);


		// Point lights
		this.gl.uniform1i(
			this.volumetricLightingShaderProgram.getUniformLocation("nrOfPointLights")[0],
		...
		for (let i = 0; i < scene.pointLights.length; i++) {
			scene.pointLights[i].bind(i, counter, this.volumetricLightingShaderProgram);
		...
		for (const pointLight of scene.pointLights) {
			if (counter - this.volumetricScreenQuad.textures.length >= pointShadowsToAllocate) {
				break;
			}
		...
		this.volumetricScreenQuad.draw(true);
		...
		if (this.blur) {
			this.combineScreenQuad.shaderProgram = this.volumetricCombineShaderProgram;
			this.volumetricCombineShaderProgram.use();
		}
		else {
			this.combineScreenQuad.shaderProgram = this.screenQuadShaderProgram;
			this.screenQuadShaderProgram.use(); // no blur
		}
		this.combineScreenQuad.draw(true);
