...
export default class Framebuffer {
	 ...
	 */
	constructor(gl: WebGL2RenderingContext, width: number, height: number, textures: Array<Texture>, depthTexture: Texture) {
		this.gl = gl;
		this.width = width;
		this.height = height;
	...
	protected setupAttachments() {
		...
		for (let i = 0; i < this.textures.length; i++) {
			...
			if (this.textures[i].textureTarget == this.gl.TEXTURE_CUBE_MAP) {
				...
				this.textures[i].setTexParameterI(this.gl.TEXTURE_MIN_FILTER, this.gl.LINEAR);
				this.textures[i].setTexParameterI(this.gl.TEXTURE_MAG_FILTER, this.gl.LINEAR);
				this.textures[i].setTexParameterI(this.gl.TEXTURE_WRAP_S, this.gl.CLAMP_TO_EDGE);
				this.textures[i].setTexParameterI(this.gl.TEXTURE_WRAP_T, this.gl.CLAMP_TO_EDGE);
				this.gl.framebufferTexture2D(
					this.gl.FRAMEBUFFER,
					this.gl.COLOR_ATTACHMENT0 + i,
					this.gl.TEXTURE_2D,
