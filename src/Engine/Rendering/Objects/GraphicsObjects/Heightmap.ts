...
export default class Heightmap extends Mesh {
	...
	private calculateVertexNormal(x: number, z: number) {
		...
		for (let i = 0; i < 4; i++) {
			...
			// prettier-ignore
			let vertex0 = vec3.fromValues(offsets[first][0] * this.xQuadSize,
				this.vertices[(z + offsets[first][1]) * this.xResolution * 8 + (x + offsets[first][0]) * 8 + 1],
				offsets[first][1] * this.zQuadSize);
			// prettier-ignore
			let vertex2 = vec3.fromValues(offsets[second][0] * this.xQuadSize,
				this.vertices[(z + offsets[second][1]) * this.xResolution * 8 + (x + offsets[second][0]) * 8 + 1],
