...
export default class Camera {
	...
	getPitchJawDegrees(): vec2 {
		return vec2.fromValues(Math.asin(this.dir[1]) * 180 / Math.PI, Math.atan2(this.dir[0], this.dir[2]) * 180 / Math.PI);
	}
	...
	setOrthogonalDimensions(left: number, right: number, bottom: number, top: number) {
		this.left = left;
		this.right = right;
		this.bottom = bottom;
		this.top = top;
	...
	private updateViewProjMatrix() {
		...
		if (this.projMatrixNeedsUpdate) {
			if (this.orthogonal) {
				mat4.ortho(this.projectionMatrix, this.left, this.right, this.bottom, this.top, 0.01, this.farPlaneDistance);
			}
			else {
	...
	bindViewProjMatrix(gl: WebGL2RenderingContext, uniformLocation: WebGLUniformLocation, skybox: boolean = false) {
		this.updateViewProjMatrix();
