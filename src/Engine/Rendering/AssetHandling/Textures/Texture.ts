export default class Texture {
	...
	constructor(
		...
		this.gl.texParameteri(this.textureTarget, this.gl.TEXTURE_WRAP_S, this.gl.REPEAT);
		this.gl.texParameteri(this.textureTarget, this.gl.TEXTURE_WRAP_T, this.gl.REPEAT);
		this.gl.texParameteri(this.textureTarget, this.gl.TEXTURE_MIN_FILTER, this.gl.NEAREST);
		this.gl.texParameteri(this.textureTarget, this.gl.TEXTURE_MAG_FILTER, this.gl.NEAREST);
	...
	setTextureData(data: Uint8Array | Float32Array, width: number, height: number) {
		this.width = width;
		this.height = height;
		this.gl.bindTexture(this.textureTarget, this.texture);
		this.gl.texImage2D(
		...
		if (this.useMipMap) {
			...
			this.gl.texParameteri(this.textureTarget, this.gl.TEXTURE_MIN_FILTER, this.gl.LINEAR_MIPMAP_LINEAR);
		}
		this.gl.bindTexture(this.textureTarget, null);
	...
	loadFromFile(URL: string) {
		...
		image.addEventListener("load", function () {
			...
			if (self.useMipMap) {
				...
				self.gl.texParameteri(self.textureTarget, self.gl.TEXTURE_MIN_FILTER, self.gl.LINEAR_MIPMAP_LINEAR);
			}
