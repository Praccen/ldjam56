...
const setPropertyWithoutTypeConversion = function(gltfNode: Object, nodeMemberName: string, object: Object, objectMemberName: string): boolean{
    if (!object.hasOwnProperty(objectMemberName)) {
        console.error("Trying to set " + objectMemberName + " on " + object.toString() + ". But object doesn't have property " + objectMemberName);
        return false;
    }
...
const setVec3 = function(gltfNode: Object, nodeMemberName: string, object: Object, objectMemberName: string): boolean{
    if (!object.hasOwnProperty(objectMemberName)) {
        console.error("Trying to set " + objectMemberName + " on object, but object doesn't have property " + objectMemberName);
        return false;
    }

    if (typeof(object[objectMemberName]) != typeof(vec3)) {
        console.error("Trying to set vec3 property " + objectMemberName + " on object, but object member is not vec3");
        return false;
    }

    if (gltfNode.hasOwnProperty(nodeMemberName) ) {
        vec3.set(object[objectMemberName], gltfNode[nodeMemberName][0], gltfNode[nodeMemberName][1], gltfNode[nodeMemberName][2]);
        return true;
    }
    return false;
}

const setQuat = function(gltfNode: Object, nodeMemberName: string, object: Object, objectMemberName: string): boolean{
    if (!object.hasOwnProperty(objectMemberName)) {
        console.error("Trying to set " + objectMemberName + " on object, but object doesn't have property " + objectMemberName);
        return false;
    }
    ...
    if (typeof(object[objectMemberName]) != typeof(quat)) {
        console.error("Trying to set quat property " + objectMemberName + " on object, but object member is not quat");
        return false;
    }

    if (gltfNode.hasOwnProperty(nodeMemberName) ) {
        quat.set(object[objectMemberName], gltfNode[nodeMemberName][0], gltfNode[nodeMemberName][1], gltfNode[nodeMemberName][2], gltfNode[nodeMemberName][3]);
        return true;
    }
    return false;
}
...
class GltfNode {
    ...
    constructor(gltfNode: any) {        
        ...
        setPropertyWithoutTypeConversion(gltfNode, "children", this, "children");5
    ...
    setMatrix(gltfNode): boolean {
        if (gltfNode.matrix != undefined && gltfNode.matrix.length == 16) {
            mat4.set(this.transform.matrix, 
    ...
    setRotation(gltfNode: any) {
        if (gltfNode.rotation != undefined) {
            quat.set(this.transform.rotation, gltfNode.rotation[0], gltfNode.rotation[1], gltfNode.rotation[2], gltfNode.rotation[3]);
            return true;
        }
        return false;
    }
...
class GltfAccessor {
    ...
    constructor(gltfAccessor: any) {
        setPropertyWithoutTypeConversion(gltfAccessor, "bufferView", this, "bufferView");
        setPropertyWithoutTypeConversion(gltfAccessor, "componentType", this, "componentType");
...
class GltfBufferView {
    ...
    constructor(gltfBufferView: any) {
        ...
        setPropertyWithoutTypeConversion(gltfBufferView, "byteLength", this, "byteLength");
        setPropertyWithoutTypeConversion(gltfBufferView, "byteOffset", this, "byteOffset");
...
class GltfAttributes {
    ...
    constructor(gltfAttribute: any) {
        setPropertyWithoutTypeConversion(gltfAttribute, "POSITION", this, "POSITION");
        setPropertyWithoutTypeConversion(gltfAttribute, "NORMAL", this, "NORMAL");
        setPropertyWithoutTypeConversion(gltfAttribute, "TEXCOORD_0", this, "TEXCOORD_0");
        setPropertyWithoutTypeConversion(gltfAttribute, "JOINTS_0", this, "JOINTS_0");
        setPropertyWithoutTypeConversion(gltfAttribute, "WEIGHTS_0", this, "WEIGHTS_0");
    }
}
...
class GltfPrimitive {
    ...
    constructor(gltfPrimitive: any) {
        ...
        setPropertyWithoutTypeConversion(gltfPrimitive, "material", this, "material");
    }
}
...
export default class GltfObject {
    ...
    private getBufferInfoFromAttribute(primitive: GltfPrimitive, attribute: string): {buffer: number, offset: number, length: number, data: Float32Array} {
        const bufferView = this.bufferViews[this.accessors[primitive.attributes[attribute]].bufferView];
        ...
        return {
            ...
            offset: offset,
            length: length,
            data: new Float32Array(this.gltfJsonContent.buffers[buffer], offset, length)
        }
    }

    private getBufferInfoForIndex(primitive: GltfPrimitive): {buffer: number, offset: number, length: number, data: Uint16Array} {
        const bufferView = this.bufferViews[this.accessors[primitive.indices].bufferView];
        let buffer = bufferView.buffer;
        let offset = bufferView.byteOffset;
        let length = bufferView.byteLength;
        return {
            ...
            offset: offset,
            length: length,
            data: new Uint16Array(this.gltfJsonContent.buffers[buffer], offset, length)
        }
    }

    getBufferData(meshIdx: number): Array<{vertexData: Float32Array, indexData: Int32Array}> {
        if (meshIdx >= this.meshes.length) {
            return null;
        }

        if (this.meshes[meshIdx].primitives == undefined && this.meshes[meshIdx].primitives.length == 0) {
            return null;
        }

        let buffers = new Array<{vertexData: Float32Array, indexData: Int32Array}>();
        ...
        for (const primitive of this.meshes[meshIdx].primitives) {
            ...
            let bufferIndex = buffers.push({vertexData: new Float32Array(numberOfVertices * 16), indexData: new Int32Array(numberOfIndices)}) - 1;

            let positionsBufferInfo = this.getBufferInfoFromAttribute(primitive, "POSITION");
            let normalBufferInfo = this.getBufferInfoFromAttribute(primitive, "NORMAL");
            let texCoordsBufferInfo = this.getBufferInfoFromAttribute(primitive, "TEXCOORD_0");
            let weightsBufferInfo = this.getBufferInfoFromAttribute(primitive, "WEIGHTS_0");
            let jointsBufferInfo = this.getBufferInfoFromAttribute(primitive, "JOINTS_0");
            ...
            for (let i = 0; i < numberOfVertices; i++) {
                ...
                for (let j = 0; j < stride; j++) {
                    buffers[bufferIndex].vertexData[i * 16 + o] = positionsBufferInfo.data[i * stride + j];
                    o++;
                }
                // vec3 inNormal
                stride = 3;
                for (let j = 0; j < stride; j++) {
                    buffers[bufferIndex].vertexData[i * 16 + o] = normalBufferInfo.data[i * stride + j];
                    o++;
                }
                // vec2 inTexCoords
                stride = 2;
                for (let j = 0; j < stride; j++) {
                    buffers[bufferIndex].vertexData[i * 16 + o] = texCoordsBufferInfo.data[i * stride + j];
                    o++;
                }
                // vec4 inWeight
                stride = 4;
                for (let j = 0; j < stride; j++) {
                    buffers[bufferIndex].vertexData[i * 16 + o] = weightsBufferInfo.data[i * stride + j];
                    o++;
                }
                // vec4 inBoneIdx; 12
                stride = 4;
                for (let j = 0; j < stride; j++) {
                    buffers[bufferIndex].vertexData[i * 16 + o] = jointsBufferInfo.data[i * stride + j];
                    o++;
                }
            }
