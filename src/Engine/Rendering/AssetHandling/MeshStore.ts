...
export default class MeshStore {
	 ...
	 */
	async loadMeshes(paths: Array<string>, progress: {loaded: number}): Promise<void> {		
	...
	async getMesh(path: string): Promise<GraphicsObject> {
		...
		if (mesh) {
			return new Promise<GraphicsObject>((resolve, reject) => {resolve(mesh)});
		}
  ...
  private async parseObjContent(meshPath: string): Promise<Float32Array> {
	...
	async parseGltfContent(meshPath: string): Promise<Array<{vertexData: Float32Array, indexData: Int32Array}>> {
		const response = await fetch(meshPath);
		let gltfContent = await response.json();

		const baseURL = new URL(meshPath, location.href);
		gltfContent.buffers = await Promise.all(gltfContent.buffers.map(async (buffer) => {
