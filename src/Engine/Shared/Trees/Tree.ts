...
export class TreeNode {
  ...
  protected checkIfContains(shape: Shape) {
    let minVec = vec3.subtract(
      ...
      this.position,
      vec3.fromValues(
        this.dimensions[0]? this.size / 2.0: InfinityLike, 
        this.dimensions[1]? this.size / 2.0: InfinityLike,  
        this.dimensions[2]? this.size / 2.0: InfinityLike
      )
    );
    let maxVec = vec3.add(
      vec3.create(),
  ...
  recalculate(
    ...
    for (let i = 0; i < this.content.length; i++) {
      ...
      if (
        this.content[i].shape.getVertexUpdateNeeded() &&
        !this.checkIfContains(this.content[i].shape)
      ) {
  ...
  getContentFromIntersection(
    ...
    if (
      IntersectionTester.identifyIntersection([intersectionShape], [this.aabb])
    ) {
      for (const child of this.children) {
  ...
  getContentForRayCast(
    ...
    if (IntersectionTester.doRayCast(ray, [this.aabb], maxDistance) < Infinity) {
      for (const child of this.children) {
